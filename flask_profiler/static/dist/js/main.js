...
var setFilteredTable = function () {
    var a = $("#filteredTable").DataTable({
        ...
        columns: [{
            title: "method", data: function (a) {
                return '<span class="row--method ' + a.method.toLowerCase() + '">' + a.method + "</span>"
            }, "class": "method", orderable: !1
        }, {
            title: "name", data: function (a, b) {
                console.log(escapeHtml(a.stats));
                var c = document.createElement("div");
                return c.innerText = a.name, "display" === b ? "<span data-json='" + escapeHtml(JSON.stringify(a.context)) + "' data-stats='" + escapeHtml(a.stats) + "'>" + c.innerHTML + "</span>" : c.innerHTML
            }, "class": "name", orderable: !1
        }, {
            title: "elapsed", data: function (a) {
                return a.elapsed.toString().slice(0, 8)
            }, "class": "elapsed number"
        }, {
            title: "startedAt", data: function (a) {
        ...
        drawCallback: function () {
            $("#filteredTable tbody").on("click", "tr", function () {
                $(".filteredModal .modal-body").JSONView(JSON.stringify($(this).find("[data-json]").data("json"))), $(".filteredModal .modal-stats").text($(this).find("[data-stats]").data("stats")), $(".filteredModal").modal("show")
            }), $("#filteredTable").removeClass("loading"), $("html").animate({scrollTop: 0}, 300)
        }
    });
    ...
    $.ajax({
        ...
        success: function (a) {
            ...
            if ("hours" === window.dayFilterValue) for (var e in Object.keys(b)) d.push(Object.keys(b)[e].substr(-2, 2)); else d = Object.keys(b);
            c3.generate({
                bindto: "#lineChart",
...
$(document).ready(function () {
    $('a[data-toggle="tab"]').historyTabs(), $("#big-users-table").on("preXhr.dt", function (a, b, c) {
    ...
    var a = $("#big-users-table").DataTable({
        ...
        serverSide: !0,
        ajax: {
            url: "api/measurements/grouped", dataSrc: function (a) {
    ...
    $(document).on("popstate", function (a) {
        ...
        window.dayFilterValue !== d, window.dayFilterValue = $(this).find("input").val(), c = "min" === window.dayFilterValue ? {
            startedAt: moment().subtract(1, "hours").unix(),
            endedAt: moment().unix()
        } : "hours" === window.dayFilterValue ? {
            startedAt: moment().subtract(24, "hours").unix(),
            endedAt: moment().unix()
        } : "days" === window.dayFilterValue ? {
            startedAt: moment().subtract(7, "days").unix(),
            endedAt: moment().unix()
        } : {
            startedAt: moment().subtract(30, "days").unix(),
            endedAt: moment().unix()
        }, window.profile.dateTime = c, getCharts(), a.draw()
    }), getCharts(), function b() {
